"""Leaflet Streamlit component setup and helpers."""
...
from nimbuschain_fetch_ui.constants import MCFG, PROJECT_ROOT

# NOTE: LEAFLET_HTML is large; keep here to avoid bloating app.py.
...
def _write_leaflet_component() -> Path:
    """Ensure the component HTML is written to a directory and return its path."""
    try:
        comp_dir = PROJECT_ROOT / "_leaflet_comp"
        comp_dir.mkdir(exist_ok=True)
        (comp_dir / "index.html").write_text(LEAFLET_HTML, encoding="utf-8")
        return comp_dir
    except Exception:
        import tempfile

        comp_dir = Path(tempfile.gettempdir()) / "sat_downloader_leaflet_comp"
        comp_dir.mkdir(parents=True, exist_ok=True)
        (comp_dir / "index.html").write_text(LEAFLET_HTML, encoding="utf-8")
        return comp_dir


_COMP_DIR = _write_leaflet_component()
...
__all__ = ["leaflet_map", "LEAFLET_HTML"]