...
from nimbuschain_zarr_service.converter.config import BandConfig, CollectionConfig
from nimbuschain_zarr_service.converter.fs import RemoteFS, is_remote_path, iter_matching_files
from nimbuschain_zarr_service.converter.utils import ensure_time, parse_timestamp_from_product_id


class Sentinel2Reader:
    """Reads Sentinel-2 products (local or remote) into an xarray.Dataset with variable `bands`."""
    ...
    def __init__(
        ...
        *,
        ...
        self.prefer_ref = prefer_ref or collection_cfg.reference_band_order or ["B04", "red"]
    ...
    def _discover_local(self, root: Path) -> dict[str, Path]:
        exts = self.cfg.extensions or {".jp2", ".tif", ".tiff"}
        candidates = list(iter_matching_files(root, extensions=exts))
        if self.cfg.subdir_filter:
            candidates = [p for p in candidates if self.cfg.subdir_filter.lower() in str(p.parent).lower()]
    ...
    def _discover_remote(self, rfs: RemoteFS) -> dict[str, str]:
        ...
        if self.cfg.subdir_filter:
            files = [f for f in files if self.cfg.subdir_filter.lower() in f.lower()]
    ...
    def _read_local(self, root: Path) -> xr.Dataset:
        ...
        if not band_map:
            raise FileNotFoundError("No Sentinel-2 bands matched local product")
        ...
        time_val = ensure_time(parse_timestamp_from_product_id(root.name, prefix="S2"))
    ...
    def _read_remote(self, uri: str) -> xr.Dataset:
        ...
        try:
            ...
            if not band_map:
                raise FileNotFoundError("No Sentinel-2 bands matched remote product")
            if self.prefetch:
            ...
            stacked = xr.concat(band_arrays, dim="band")
            stacked = stacked.assign_coords(band=band_names)
            time_val = ensure_time(parse_timestamp_from_product_id(uri.split("/")[-1], prefix="S2"))
            stacked = stacked.expand_dims(time=[time_val])
            ds = xr.Dataset({"bands": stacked})
            ds["bands"].rio.write_crs(ref_da.rio.crs, inplace=True)
            ds["bands"].rio.write_transform(ref_da.rio.transform(), inplace=True)
