...
def test_converter_health_returns_503_when_zarr_service_url_is_missing() -> None:
    app = FastAPI()
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == "Zarr service URL is not configured."


def test_mask_health_returns_503_when_mask_service_url_is_missing() -> None:
    app = FastAPI()
    app.state.settings = Settings(NIMBUS_MASK_SERVICE_URL="")
    app.include_router(converter_router)
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == "Mask service URL is not configured."
