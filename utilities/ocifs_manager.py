...
class OCIFSManager(object):
    ...
    def __init__(self, bucket: str, config_path: str = "~/.oci/config", profile: str = "DEFAULT"):
        """
        ...
        if not stc.valid():
            logger.exception(f"CLI token has expired, please re-authenticate!")
            sys.exit(1)
        ...
        self.object_storage_client = oci.object_storage.ObjectStorageClient(config=self.config, signer=self.signer)
        compartment_ocid = os.environ.get("COMPARTMENT_OCID")
        if not compartment_ocid:
            logger.exception(f"COMPARTMENT_OCID environment variable is not set.")
            sys.exit(1)
        self.namespace = self.object_storage_client.get_namespace(compartment_id=compartment_ocid).data
